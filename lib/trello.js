  ...
  try {
    ...
    for (const nalog of statusList) {
      ...
      for (const it of nalog.items) {
        ...
        // skeniranje (skeniranje je vezano uz plastifikaciju/boju) - za CNC
        // naloge, ako je Trello (stroj) označio poziciju kao gotovu, tretiramo
        // je kao potpuno gotovu bez obzira na (nepostojeći) sken.
        if (!it.finishing && it.cncGotovo === true) {
          it.statusSkeniranja = 'potpuno';
          it.skenirano = true;
          if (typeof it.qty === 'number') {
            it.komadaSkenirano = Math.max(it.komadaSkenirano || 0, it.qty);
          }
        }
      }
    }
