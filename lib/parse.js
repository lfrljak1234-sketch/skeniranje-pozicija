...
function computeStatus(nalozi, skenoviByKey) {
  ...
  for (const nalogBase of Object.keys(nalozi)) {
    ...
    const itemsWithStatus = nalog.items.map(it => {
      ...
      if (!isFinSoffit || linkedNsNalozi.length === 0 || !isCncDept(nalog.odjel)) {
  ...
  // --- Prolaz 2: zbroji skenirane komade po (projekt, Part Number) kroz SVE RN naloge ---
  // (za ASL "Production Plan" usporedbu - ista pozicija zna biti raspoređena
  // kroz više RN naloga istog projekta)
  const scannedByProjectPart = new Map(); // "projektKey|PARTNUMBER" -> zbroj komada
  for (const nalogBase of Object.keys(nalozi)) {
    const nalog = nalozi[nalogBase];
    if (nalog.format !== 'RN') continue;
    const projectKey = opisMatchKey(nalog.opis);
    if (!projectKey) continue;
    for (const it of (itemsByNalog.get(nalogBase) || [])) {
      if (!it.partNumber) continue;
      const key = projectKey + '|' + it.partNumber.toUpperCase();
      scannedByProjectPart.set(key, (scannedByProjectPart.get(key) || 0) + (it.komadaSkenirano || 0));
    }
  }

  // --- Prolaz 3: sastavi konačni rezultat, uključujući ASL plan status ---
  const result = [];
  for (const nalogBase of Object.keys(nalozi)) {
    const nalog = nalozi[nalogBase];
    const itemsWithStatus = itemsByNalog.get(nalogBase) || [];

    let productionPlanStatus = null;
    if (nalog.productionPlan) {
      const projectKey = opisMatchKey(nalog.opis);
      productionPlanStatus = nalog.productionPlan.map(pe => {
        const key = projectKey + '|' + pe.partNumber.toUpperCase();
        const skenirano = scannedByProjectPart.get(key) || 0;
        const status = statusOd(skenirano, pe.total);
        return { ...pe, skenirano, status };
      });
    }
