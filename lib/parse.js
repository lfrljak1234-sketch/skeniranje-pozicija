...
const { extractRalCode, extractColorName, colorForFinishing } = require('./ral-colors');
...
function parseNalogWorkbook(buffer, originalFilename) {
  ...
  for (let i = headerIdx + 1; i < rows.length; i++) {
    ...
    items.push({
      ...
      colorName: extractColorName(finishing) || (extractRalCode(finishing) ? 'RAL ' + extractRalCode(finishing) : null),
...
function parseSkenoviCsv(buffer) {
  ...
  function bump(key, rec) {
    ...
    // broj komada pa se jednom skenira barkod) - zbrajamo stvarni broj
