...
// Pozicije čiji naziv/šifra sadrži "fin" ili "soffit" fizički se skeniraju
// preko NS (narudžba materijala) sustava, ne preko RN-ovog vlastitog
// numeriranja pozicija - njihov barkod treba potražiti u odgovarajućem NS
// dokumentu (onom čiji "opis" upućuje na isti RN nalog).
...
function computeStatus(nalozi, skenoviByKey) {
  ...
  for (const nalogBase of Object.keys(nalozi)) {
    ...
    const itemsWithStatus = nalog.items.map(it => {
      let scanKeys = it.scanKeys || [];
      if (linkedNsNalozi.length && it.partNumber && FIN_SOFFIT_RE.test((it.partNumber || '') + ' ' + (it.description || ''))) {
        const extra = [];
        for (const nsNalog of linkedNsNalozi) {
          ...
          if (match && match.scanKeys && match.scanKeys.length) extra.push(...match.scanKeys);
        }
        if (extra.length) scanKeys = [...scanKeys, ...extra];
      }
