...
function computeStatus(nalozi, skenoviByKey, dualPhaseKeywords) {
  ...
  for (const nalogBase of Object.keys(nalozi)) {
    ...
    const itemsWithStatus = nalog.items.map(it => {
      ...
      const isFinSoffit = !!(it.partNumber && dualPhaseRe && dualPhaseRe.test((it.partNumber || '') + ' ' + (it.description || '')));
...
module.exports = { extractRnBase, parseNalogWorkbook, parseSkenoviCsv, computeStatus, applyProductionPlanStatus, DEFAULT_DUALPHASE_KEYWORDS };
