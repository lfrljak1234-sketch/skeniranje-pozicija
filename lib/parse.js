...
function parseSkenoviCsv(buffer) {
  ...
  // CSV zna imati POMIJEŠANE završetke redaka (dio redaka \r\n, dio samo \n) -
  // ako se to ne normalizira, csv-parse pogodi JEDAN stil kao standard i
  // retke s drugim stilom tiho spoji u susjedni redak (izgubljeni podaci,
  // bez ikakve greške). Normaliziramo sve na \n prije parsiranja.
  text = text.replace(/\r\n/g, '\n').replace(/\r/g, '\n');
  let records;
  try {
...
function computeStatus(nalozi, skenoviByKey, dualPhaseKeywords, preOnlyKeywords) {
  ...
  for (const nalogBase of Object.keys(nalozi)) {
    ...
    const itemsWithStatus = nalog.items.map(it => {
      ...
      } else if (!isFinSoffit || !primjenjivo) {
      ...
      // Pozicije BEZ definirane boje (Finishing prazan) ne prolaze kroz
      // skeniranje (skeniranje je vezano uz plastifikaciju/boju) - za njih,
      // ako su SSP koraci proizvodnje u potpunosti odrađeni (gotovoKolicina
      // >= potrebna količina na svakom koraku), tretiramo poziciju kao
      // potpuno gotovu bez obzira na (nepostojeći) sken.
      if (!it.finishing && it.techSteps && it.techSteps.length > 0 && potrebnaKolicina !== null) {
        const sviKoraciGotovi = it.techSteps.every(s => s.gotovoKolicina != null && s.gotovoKolicina >= potrebnaKolicina);
        if (sviKoraciGotovi) {
          result.statusSkeniranja = 'potpuno';
          result.skenirano = true;
          result.komadaSkenirano = Math.max(result.komadaSkenirano, potrebnaKolicina);
        }
      }
